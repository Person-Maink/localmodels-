...
def _lowpass_filter(signal: np.ndarray, fps: float, cutoff: float = 6.0, order: int = 3) -> np.ndarray:
    min_length = max(8, 3 * max(order, 1) + 3)
...
def _bandpass_filter(signal: np.ndarray, fps: float, low: float = 2.0, high: float = 12.0, order: int = 3) -> np.ndarray:
    min_length = max(8, 3 * max(order, 1) + 3)
    if signal.shape[0] <= min_length:
        return np.asarray(signal, dtype=np.float32)
    nyq = 0.5 * float(fps)
    b_high, a_high = butter(order, low / nyq, btype="high")
    try:
        signal = filtfilt(b_high, a_high, signal, axis=0)
    except ValueError:
        return np.asarray(signal, dtype=np.float32)
    b_low, a_low = butter(order, high / nyq, btype="low")
    try:
        return filtfilt(b_low, a_low, signal, axis=0)
    except ValueError:
        return np.asarray(signal, dtype=np.float32)


def _finish_analysis(trajectory: Sequence[np.ndarray], fps: float, filter_kind: str = "lowpass") -> dict:
    if len(trajectory) < 8:
        raise RuntimeError("Not enough frames in the selected segment. Choose a longer range.")
    trajectory_array = np.stack(trajectory, axis=0)
    if filter_kind == "bandpass":
        filtered = _bandpass_filter(trajectory_array, fps=fps)
    else:
        filtered = _lowpass_filter(trajectory_array, fps=fps)

    magnitude = np.linalg.norm(filtered, axis=1)
    magnitude = magnitude - magnitude.mean()
    freqs, psd = welch(magnitude, fs=float(fps), nperseg=min(256 if filter_kind == "lowpass" else 512, len(magnitude)))
...
def _analyze_centroid(source: dict, frames: List[dict], hand_value: int, wrist_joint_idx: int, fps: float) -> dict:
    ...
    return _finish_analysis(centroids, fps=fps, filter_kind="bandpass")


def _build_regions(n_neighbors: int, vertex_a: int, vertex_b: int, settings: AppSettings) -> Tuple[np.ndarray, np.ndarray]:
    mano = load_mano_assets(str(settings.mano_right_path))
    faces = mano["faces_right"]
    adjacency = _build_vertex_adjacency(int(mano["j_regressor"].shape[1]), faces)
    return _build_region_indices(vertex_a, adjacency, n_neighbors), _build_region_indices(vertex_b, adjacency, n_neighbors)
...
def _analyze_point_to_point(source: dict, frames: List[dict], hand_value: int, wrist_joint_idx: int, fps: float, vertex_a: int, vertex_b: int, n_neighbors: int, mediapipe_point_a: int, mediapipe_point_b: int) -> dict:
    ...
    if source["family"] == "mediapipe":
        ...
        return _finish_analysis(trajectory, fps=fps)

    region_a, region_b = _build_regions(n_neighbors, vertex_a, vertex_b, settings=_analyze_point_to_point.settings)
    for frame in frames:
        hands = _selected_hands(frame, hand_value)
        if not hands:
            continue
...
def _analyze_camera_space(source: dict, frames: List[dict], hand_value: int, fps: float, n_neighbors: int, mano_pairs: Sequence[Tuple[int, int]], settings: AppSettings) -> List[dict]:
    region_map = [_build_regions(n_neighbors, pair[0], pair[1], settings) for pair in mano_pairs]
    entries = []
    for pair, (region_a, region_b) in zip(mano_pairs, region_map):
        trajectory = []
        for frame in frames:
            hands = _selected_hands(frame, hand_value)
            if not hands:
            ...
            trajectory.append(np.mean(np.stack(frame_diffs, axis=0), axis=0))
        result = _finish_analysis(trajectory, fps=fps)
        entries.append({"pair_label": f"v{pair[0]}-v{pair[1]}", "result": result})
    return entries
...
def _beta_variant_entries(source: dict, settings: AppSettings, hand_value: int, wrist_joint_idx: int, vertex_a: int, vertex_b: int, n_neighbors: int, fps: float, frame_start: Optional[int], frame_end: Optional[int]) -> List[dict]:
    region_a, region_b = _build_regions(n_neighbors, vertex_a, vertex_b, settings)
        ...
        for frame in frames:
            ...
            for hand in hands:
                if "joints_world" in hand:
                    ...
                    centered = np.asarray(hand["verts_world"], dtype=np.float32)
                frame_diffs.append(centered[region_a].mean(axis=0) - centered[region_b].mean(axis=0))
            trajectory.append(np.mean(np.stack(frame_diffs, axis=0), axis=0))
        entries.append({"label": label, "result": _finish_analysis(trajectory, fps=fps)})
    return entries
...
def _compare_figure(title: str, entries: List[dict], fps: float):
    ...
    for index, entry in enumerate(entries):
        ...
        for axis_index, axis_name in enumerate(["x", "y", "z"]):
            axes[2].plot(t, result["filtered"][:, axis_index], color=color, lw=1.1, alpha=0.35 + 0.2 * axis_index, label=f"{entry['label']} {axis_name}")
    ...
    axes[1].grid(True)
    axes[1].legend()
    axes[2].set_title("Filtered XYZ displacement")
...
class AnalysisService:
    ...
    def run(self, mode_id: str, sources: List[dict], frame_ranges: Dict[str, Tuple[Optional[int], Optional[int]]], params: dict) -> Tuple[dict, str]:
        ...
        if mode_id == "beta_comparison":
            ...
            svg = figure_to_svg(_compare_figure("Raw vs beta-average comparison", entries, fps))
            return (
                {

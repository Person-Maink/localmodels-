...
from ..gcn import _GraphConv
from ..gcn import adj_mx_from_edges, get_sketch_setting
import einops
...
class Decorder_VSSM(VSSM):
    def __init__(self, norm_layer="ln", **kwargs):
        kwargs.update(norm_layer=norm_layer)
        super().__init__(**kwargs)
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"])
        _NORMLAYERS = dict(
        ...
        self.out_indices = [i for i in range(len(dims))]
...
class Decorder_GCN_VSSM(VSSM):
    def __init__(self, norm_layer="ln", joint_num=21, **kwargs):
        kwargs.update(norm_layer=norm_layer)
        super().__init__(**kwargs)
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"])
        _NORMLAYERS = dict(

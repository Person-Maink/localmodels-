...
ARRAY_TEMPLATE="${ARRAY_TEMPLATE:-${SCRIPT_DIR}/template_hamba_array.sh}"
...
while IFS= read -r bucket_label; do
    ...
    echo "Submitting hamba bucket ${bucket_time} with ${task_count} videos"
    submit_batch_script "${array_script}"
    submitted_arrays=$((submitted_arrays + 1))
done < <(printf '%s\n' "${!BUCKET_MANIFESTS[@]}" | sort)

echo "Prepared ${video_count} hamba videos across ${submitted_arrays} array submissions"

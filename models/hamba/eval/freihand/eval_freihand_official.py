...
def verts2pcd(verts, color=None):
    ## https://github.com/isl-org/Open3D/issues/2630
    # pcd = o3d.PointCloud()
...
def calculate_fscore(gt, pr, th=0.01):
    ...
    # See this issue to fix below problem: https://github.com/isl-org/Open3D/issues/2389
    # Cross-Checked with [0,0,0] keypoint input and new function matches with old function results.
...
def createHTML(outputDir, curve_list):
    ...
    for item in curve_list:
        ...
        with open(img_path, 'rb') as f:
            image_data = f.read()
        data_uri1 = base64.b64encode(image_data).decode('utf-8').replace('\n', '')
...
def main(gt_path, pred_path, output_dir, pred_file_name=None, set_name=None):
    ...
    if pred_file_name[-3:] == "zip":
        pred_file_path = os.path.join(pred_path, pred_file_name)
        print('!!!ZIP, Loading predictions from zip file: %s' % pred_file_path)
        pred = load_pred_json(pred_file_path)
    else:
        pred_file = _search_pred_file(pred_path, pred_file_name)
        print('!!!JSON, Loading predictions from JSON file: %s' % pred_file)
    ...
    for idx in rng:
        ...
        # align predictions
        # print("xyz.shape, xyz_pred.shape: ", xyz.shape, xyz_pred.shape)
        if len(xyz_pred.shape) == 3 and xyz_pred.shape[0] == 1:
            xyz_pred = xyz_pred[0]
            verts_pred = verts_pred[0]
    ...
    score_path = os.path.join(output_dir, 'pck_data.json')
    with open(score_path, 'w') as fo:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gt_dir', type=str,
                        default='datasets/hand_vim_eval_data',
    ...
    parser.add_argument('--pred_file_name', type=str, 
                        # default='pred.json',
                        default='hamba-ckptxx-sc-rot-final-pred.zip',

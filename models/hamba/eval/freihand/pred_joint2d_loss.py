...
from hamba.models.losses import Keypoint3DLoss, Keypoint2DLoss, ParameterLoss
...
def main(base_path, pred_out_path, model, init_regression_model=None, set_name=None):
    ...
    joint2d_pred_list, joint2d_gt_list = list(), list()
    ...
    for idx in tqdm(range(db_size(set_name))):
        ...
        xyz, verts, joint2d_pred, joint2d_gt = get_xyz_verts(
            img,
            model,
        ...
        joint2d_pred_list.append(joint2d_pred)
        joint2d_gt_list.append(joint2d_gt)

    ## compute 2d joint loss
    joint2d_loss_list = []
    for i in range(len(joint2d_pred_list)):
        joint2d_pred = joint2d_pred_list[i]
        joint2d_gt = joint2d_gt_list[i]

        joint2d_loss = keypoint_2d_loss(joint2d_pred, joint2d_gt)
        joint2d_loss_list.append(joint2d_loss)
    joint2d_loss_all = torch.stack(joint2d_loss_list)
    joint2d_loss_mean = joint2d_loss_all.mean()
    
    # dump results
    # dump(pred_out_path, xyz_pred_list, verts_pred_list)
    
    return joint2d_loss_mean
...
def get_xyz_verts(img, model, init_regression_model, K, scale, gt_keypoints_3d):
    ...
    IMG_SIZE = 224
    batch = {
        'img': torch.from_numpy(img_patch).float(), # the input img is RGB order, not BGR(cv2)
    ...
    return xyz, verts, out['pred_keypoints_2d'], batch['keypoints_2d']
...
if __name__ == '__main__':
    ...
    checkpoint = "logs/train/run/hamba.ckpt"

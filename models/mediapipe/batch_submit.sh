...
TEMPLATE="${TEMPLATE:-${SCRIPT_DIR}/template_mediapipe.sh}"
OUT_DIR="${OUT_DIR:-${SCRIPT_DIR}/generated_jobs}"
LOG_DIR="${LOG_DIR:-${SCRIPT_DIR}/SLURM_logs}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${SCRIPT_DIR}/../../outputs/mediapipe}"
...
for video in "${videos[@]}"; do
    ...
    escaped_filename=$(escape_sed_replacement "${filename}")
    escaped_time=$(escape_sed_replacement "${final_time}")

    sed \
        -e "s|__NAME__|${escaped_name}|g" \
        -e "s|__VIDEO_FILE__|${escaped_filename}|g" \
...
echo "Prepared ${video_count} mediapipe job scripts in ${OUT_DIR}"
echo "Submitted ${submitted_count} mediapipe jobs"
echo "Skipped ${skipped_count} completed mediapipe videos"

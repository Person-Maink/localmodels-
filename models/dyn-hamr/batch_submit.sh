...
TEMPLATE="${TEMPLATE:-${SCRIPT_DIR}/template_dynhamr.sh}"
ARRAY_TEMPLATE="${ARRAY_TEMPLATE:-${SCRIPT_DIR}/template_dynhamr_array.sh}"
...
while IFS= read -r bucket_label; do
    ...
    echo "Submitting dyn-hamr bucket ${bucket_time} with ${task_count} videos"
    submit_batch_script "${array_script}"
    submitted_arrays=$((submitted_arrays + 1))
done < <(printf '%s\n' "${!BUCKET_MANIFESTS[@]}" | sort)

echo "Prepared ${video_count} dyn-hamr videos across ${submitted_arrays} array submissions"

...
from hmp_stride_adapter_utils import (process_gt, gmof, perspective_projection, get_joints2d, run_pymafx, run_metro, process_pymafx_mano, compute_seq_intervals,
...
try:
    ...
    bmc = BMCLoss(lambda_bl=0, lambda_rb=0, lambda_a=0)
...
def get_stage2_res(base_dir, device, npz_init_dict, hand_model):
    args = Arguments(base_dir, str(HMP_CLEAN_PATH), filename='amass.yaml')
...
def multi_stage_opt(opt, device, obs_data, res_dict, hand_model, config_f, exp_setup_name, init_method_name):
    ...
    hmp_args = Arguments(
        opt.paths.base_dir,
        str(HMP_CLEAN_PATH),
        filename=opt.HMP.config,
        mano_dir=getattr(opt.HMP, "mano_dir", None),
    )
...
def fitting_prior(obs_data, res_dict, hand_model, opt, data_args, out_dir, device):
    ...
    if hasattr(opt.HMP, "mano_dir") and opt.HMP.mano_dir:
        args.smpl.smpl_body_model = opt.HMP.mano_dir
    ...
    args.dataset_dir = getattr(opt.HMP, "assets_root", os.path.join(opt.paths.base_dir, '_DATA/hmp_model'))
    args.save_dir = getattr(opt.HMP, "assets_root", os.path.join(opt.paths.base_dir, '_DATA/hmp_model'))
    ...
    fk = ForwardKinematicsLayer(args)
    multi_stage_opt(opt, device, obs_data, res_dict, hand_model, os.path.join(HMP_CLEAN_PATH, opt.HMP.config), opt.HMP.exp_name, init_method)

...
VIDEO_NAME="${VIDEO_NAME:-me 1_contrast_100_amplified}"
VIDEO_FILE="${VIDEO_FILE:-}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-lora_finetuning}"
...
python main.py --image_folder $(printf '%q' "${VIDEO_DIR}") --frame_cache_root $(printf '%q' "${FRAME_CACHE_ROOT}") --video $(printf '%q' "${VIDEO_STEM}") --output_folder $(printf '%q' "${OUTPUT_ROOT}") --checkpoint_path $(printf '%q' "${CHECKPOINT_PATH}") --cfg_path $(printf '%q' "${CFG_PATH}") ${visualize_flag} ${save_mesh_flag} ${gpu_flag}
EOF
)
...
echo "WiLoR frame cache root: ${FRAME_CACHE_ROOT}"

...
HMP_CLEAN_PATH = Path(__file__).resolve().parent / "hmp_clean"
if str(HMP_CLEAN_PATH) not in sys.path:
    sys.path.insert(0, str(HMP_CLEAN_PATH))
...
class BMCLoss:
    def __init__(
        ...
        candidates = [
            Path(__file__).resolve().parent / "BMC",
            Path(__file__).resolve().parent / "_DATA" / "BMC",
            Path(__file__).resolve().parent.parent / "_DATA" / "BMC",
        ]
        self.lp = None
        for candidate in candidates:
            if candidate.exists():
                self.lp = str(candidate)
                break
        if self.lp is None:
            raise FileNotFoundError(
                "Could not locate optional BMC assets. Checked: "

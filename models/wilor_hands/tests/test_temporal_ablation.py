...
class TemporalAblationTests(unittest.TestCase):
    ...
    def test_resolve_experiment_rejects_removed_temporal_vipe_family(self) -> None:
        payload = {
            "experiments": [
                {
                    "name": "removed_family",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(config_path, "w", encoding="utf-8") as handle:
                yaml.safe_dump(payload, handle, sort_keys=False)
            with self.assertRaisesRegex(ValueError, "Unknown loss family"):
                resolve_experiment_config(config_path, "removed_family")
    ...
    def test_env_map_exports_optimizer_values(self) -> None:
        ...
        self.assertEqual(env_map["TEMPORAL_CAMERA_ENABLED"], "true")
        self.assertEqual(env_map["TEMPORAL_BBOX_PROJECTED_ENABLED"], "true")
        self.assertNotIn("TEMPORAL_VIPE_CAMERA_ENABLED", env_map)
        self.assertNotIn("TEMPORAL_VIPE_CAMERA_SMOOTHNESS_WEIGHT", env_map)
        self.assertNotIn("TEMPORAL_VIPE_CAMERA_ANCHOR_WEIGHT", env_map)
    ...
    @unittest.skipUnless(TORCH_AVAILABLE, "torch is not installed")
    def test_temporal_camera_loss_bundle_reports_surviving_metrics(self) -> None:
        student_output_seq = {
        ...
        self.assertIn("loss_temporal_camera_base", metrics)
        self.assertIn("loss_temporal_camera_score", metrics)
        self.assertNotIn("loss_temporal_vipe_camera_align", metrics)
    ...
    @unittest.skipUnless(TORCH_AVAILABLE, "torch is not installed")
    def test_removed_trainable_scopes_raise_value_error(self) -> None:
        class _TinyModel(torch.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.backbone = torch.nn.Module()
        ...
        model = _TinyModel()
        with self.assertRaisesRegex(ValueError, "Unsupported train scope"):
            configure_trainable_scope(model, "camera_head")
        with self.assertRaisesRegex(ValueError, "Unsupported train scope"):
